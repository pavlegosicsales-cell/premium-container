/* ==========================================================================
   Premium Container, ponasanje sajta
   Mobilna navigacija, scroll reveal, podela naslova po recima, FAQ
   akordeon, visekoraona forma za upit i footer forma za povratni poziv.
   ========================================================================== */

/* Ovde ide Apps Script URL nakon Skilla 03 (Form Backend Setup) */
const ENDPOINT = '';
...
(function () {
  ...
  function send(payload) {
    if (!ENDPOINT) {
      ...
      console.info('[Premium Container] ENDPOINT je prazan. Ovo bi bilo poslato:', payload);
  ...
  if (form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      submitBtn.firstChild.textContent = 'Šaljem ';

      const payload = {
        formType: 'Upit za ponudu, Premium Container',
      ...
      send(payload).then(function () {
        ...
        summaryEl.innerHTML = '';
        [
          ['Objekat', answers.service],
          ['Veličina', answers.roofType],
          ['Oprema', answers.condition],
          ['Region', answers.region],
          ['Rok', answers.timing],
        ...
        if (formError) {
          ...
          formError.textContent = 'Slanje nije uspelo. Pozovite 061 743 6390 i rešićemo.';
        }
      });
    });
  ...
  if (cbForm) {
    ...
    cbForm.addEventListener('submit', function (e) {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
        cbNote.textContent = 'Upišite ispravnu imejl adresu.';
        cbInput.focus();
        return;
      }

      cbBtn.disabled = true;
      cbBtn.textContent = 'Šaljem';
      cbNote.textContent = '';
      ...
      send({
        ...
        cbNote.textContent = 'Hvala, javljamo se uskoro telefonom.';
      }).catch(function (err) {
        console.error(err);
        cbBtn.textContent = 'Pozovite me';
        cbBtn.disabled = false;
        cbNote.textContent = 'Slanje nije uspelo. Pozovite 061 743 6390.';
      });
    });
  }
